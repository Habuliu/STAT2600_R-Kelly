...
ggplot(delay_data)+geom_col(mapping = aes(x=delaytype, y=total_time_delayed))
```
...
ggplot(delay_data)+geom_col(mapping = aes(x=delaytype, y=amount_of_delays))
```
...
###Lauren
I examined the probability of different delay types given that a flight was delayed. 
