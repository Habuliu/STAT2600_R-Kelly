...
Ping_Austin <- filter(babynamesAustin,str_detect(babynamesAustin$name, fixed("Aus"))) %>% group_by(year)
Ping_Austin2 <- Ping_Austin %>% filter(name == "Austin") %>% group_by(year) %>% summarize(sumProp2 = sum(prop))
Ping_Austin <- Ping_Austin %>% summarize(sumProp = sum(prop))

ggplot(data = Ping_Austin, mapping = aes(x = year, y = sumProp)) + geom_line(col = "red") + ylab("Sum of Proportion") + geom_line(data = Ping_Austin2, mapping = aes(x = year, y = sumProp2), col = "blue") + ggtitle("The proportion of babys' name is 'Austin' and start with 'Aus' from 1880 to 2001")
```
