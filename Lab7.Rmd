...
```{r}
powerConsum <- powerConsum %>% mutate(Year = year(dmy(Date)), Month = month(dmy(Date)), Day = day(dmy(Date)))

Jan <- powerConsum %>% filter(powerConsum$Month == 1, powerConsum$Year == 2008) + select(powerConsum$Sub_metering_3, powerConsum$Year, powerConsum$Month)
Feb <- powerConsum %>% filter(powerConsum$Month == 2, powerConsum$Year == 2008) + select(powerConsum$Sub_metering_3, powerConsum$Year, powerConsum$Month)
Mar <- powerConsum %>% filter(powerConsum$Month == 3, powerConsum$Year == 2008) + select(powerConsum$Sub_metering_3, powerConsum$Year, powerConsum$Month)
Apr <- powerConsum %>% filter(powerConsum$Month == 4, powerConsum$Year == 2008) + select(powerConsum$Sub_metering_3, powerConsum$Year, powerConsum$Month)
May <- powerConsum %>% filter(powerConsum$Month == 5, powerConsum$Year == 2008) + select(powerConsum$Sub_metering_3, powerConsum$Year, powerConsum$Month)
Jun <- powerConsum %>% filter(powerConsum$Month == 6, powerConsum$Year == 2008) + select(powerConsum$Sub_metering_3, powerConsum$Year, powerConsum$Month)
Jul <- powerConsum %>% filter(powerConsum$Month == 7, powerConsum$Year == 2008)+ select(powerConsum$Sub_metering_3, powerConsum$Year, powerConsum$Month)
Aug <- powerConsum %>% filter(powerConsum$Month == 8, powerConsum$Year == 2008) + select(powerConsum$Sub_metering_3, powerConsum$Year, powerConsum$Month)
Sep <- powerConsum %>% filter(powerConsum$Month == 9, powerConsum$Year == 2008) + select(powerConsum$Sub_metering_3, powerConsum$Year, powerConsum$Month)
Oct <- powerConsum %>% filter(powerConsum$Month == 10, powerConsum$Year == 2008)+ select(powerConsum$Sub_metering_3, powerConsum$Year, powerConsum$Month)
Nov <- powerConsum %>% filter(powerConsum$Month == 11, powerConsum$Year == 2008)+ select(powerConsum$Sub_metering_3, powerConsum$Year, powerConsum$Month)
Dec <- powerConsum %>% filter(powerConsum$Month == 12, powerConsum$Year == 2008)+ select(powerConsum$Sub_metering_3, powerConsum$Year, powerConsum$Month)
...
ggplot(data = Graph) + geom_line(mapping = aes(x = Graph$Month, y = Graph$Total)) + labs(x = "Month", y = "Meter3 total", title = "Water-heater & Air-conditioner usage in 2008") + scale_x_continuous(breaks=c(1,3,5,7,9,11))
```
